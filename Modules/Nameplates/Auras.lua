        ...
        if icon then
            ...
            if auraData then
                ...
                if not skipDueToHidePermanent then
                    ...
                    -- Duration text (countdown number)
                    -- Note: Text won't update continuously without cooldown sweep
                    -- The cooldown frame handles the animation, text is just initial display
                    local showDurationText = config.showDurationText
                    if showDurationText == nil then showDurationText = true end
                    
                    if showDurationText and icon.durationText then
                        icon.durationText:SetFont("Fonts\\FRIZQT__.TTF", 9, "OUTLINE")
                        ...
                        local remaining = TweaksUI.API.GetAuraDurationRemaining(unit, auraID)
