...
local function CreateAuraIcon(container, index)
    ...
    icon:EnableMouse(true)  -- For tooltips only - Frame doesn't intercept clicks like Button does
    ...
    -- OnUpdate for duration text (throttled to 0.1s for performance)
    icon.durationElapsed = 0
    icon:SetScript("OnUpdate", function(self, elapsed)
        -- Throttle updates to 10 FPS (0.1s) instead of every frame
        self.durationElapsed = (self.durationElapsed or 0) + elapsed
        if self.durationElapsed < 0.1 then return end
        self.durationElapsed = 0
        
        if not self.showDurationText or not self.expirationTime then
        ...
        if remaining and remaining > 0 then
            ...
            if remaining >= 3600 then
                text = string.format("%dh", math.floor(remaining / 3600))
...
local function UpdateAuraIcon(icon, unit, auraInstanceID, auraData, settings)
    ...
    -- Store expiration time for duration text OnUpdate
    icon.expirationTime = nil
    icon.showDurationText = false
    
    if settings.showDurationText and icon.durationText then
        local hasExpiration = AuraAPI and AuraAPI:HasExpirationTime(unit, auraInstanceID)
        
        if hasExpiration then
            -- Try to get expiration time from aura data
            pcall(function()
            ...
            if icon.expirationTime then
                icon.showDurationText = true
                icon.durationText:Show()
                
                -- Set up OnUpdate for duration text if not already done (throttled for performance)
                if not icon.durationUpdateScript then
                    icon.durationUpdateScript = true
                    icon.durationElapsed = 0
                    icon:SetScript("OnUpdate", function(self, elapsed)
                        -- Throttle updates to 10 FPS (0.1s) instead of every frame
                        self.durationElapsed = (self.durationElapsed or 0) + elapsed
        ...
        if not textureSet then
            ...
            if shouldDisplay then
                ...
                if icon.durationText then
                    ...
                    if settings.showDurationText and AuraAPI and auraInstanceID then
                        local hasExpiration = AuraAPI:HasExpirationTime(unit, auraInstanceID)
                        if hasExpiration then
                            pcall(function()
                                icon.expirationTime = auraData.expirationTime
