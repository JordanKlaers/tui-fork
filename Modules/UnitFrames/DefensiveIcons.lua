...
-- Falls back to Blizzard's CenterDefensiveBuff cache, then direct scanning
...
function DefensiveIcons:FindDefensiveAura(unit)
    ...
    -- Method 1: Use AuraAPI with BigDefensive sort rule
    if AuraAPI then
        local auras = AuraAPI:GetUnitAuras(unit, "HELPFUL", Enum.UnitAuraSortRule.BigDefensive)
        
        if auras and #auras > 0 then
            local auraData = auras[1]  -- First aura is the "big defensive"
            if auraData then
                -- Check if it has an icon we can display
                local hasIcon = false
                pcall(function()
                    hasIcon = auraData.icon ~= nil
                end)
    ...
    -- Method 2: Check if Blizzard has cached a defensive for this unit
    local cachedAuraID = self:GetCachedDefensiveID(unit)
    
    if cachedAuraID then
        -- Use Blizzard's chosen defensive - this is Midnight-safe!
    ...
    -- Method 3: Fallback direct scanning
    return ScanForDefensiveAura(unit)
end
...
function DefensiveIcons:UpdateDefensiveIcon(frame, settings)
    ...
    -- Duration text display (like aura icons)
    local showDurationText = settings.showDurationText
    if showDurationText and auraInstanceID and icon.durationText then
        -- Get remaining duration using TweaksUI.API
        local remaining = nil
        if TweaksUI and TweaksUI.API and TweaksUI.API.GetAuraDurationRemaining then
            remaining = TweaksUI.API.GetAuraDurationRemaining(unit, auraInstanceID)
        end
        
        if remaining then
            -- Use Midnight's TruncateWhenZero if available
