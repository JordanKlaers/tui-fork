...
local function GetUninterruptibleColorCurve()
    ...
    if CreateColorCurve then
        uninterruptibleColorCurve = CreateColorCurve()
        -- 0.0 (false) = white (1,1,1) - means "use normal color"
        -- 1.0 (true) = grey (0.7,0.7,0.7) - uninterruptible
        uninterruptibleColorCurve:AddPoint(0, 1, 1, 1, 1)
        uninterruptibleColorCurve:AddPoint(1, 0.7, 0.7, 0.7, 1)
    end
...
local function ApplyCastBarColor(statusBar, notInterruptible, baseR, baseG, baseB, isPlayerUnit)
    ...
    -- For non-player units, notInterruptible might be secret
    ...
    -- Value is secret - try color curve API
    if HAS_COLOR_FROM_BOOLEAN and notInterruptible ~= nil then
        local curve = GetUninterruptibleColorCurve()
        if curve then
            local ok, colorResult = pcall(C_CurveUtil.EvaluateColorFromBoolean, curve, notInterruptible)
            if ok and colorResult then
                local r, g, b, a = colorResult:GetRGBA()
                if r and r < 0.9 then
                    -- It's grey - uninterruptible
                    statusBar:SetStatusBarColor(r, g, b, a or 1)
                    return true
                else
                    -- It's white - use base color
                    statusBar:SetStatusBarColor(baseR, baseG, baseB, 1)
                    return false
                end
            end
...
local function InitRangeChecker()
    -- NO-OP: RangeFade module now handles all range checking
...
local function IsPreviewEnabled(unit)
    if unit == "target" then return previewTarget
    ...
    return false  -- Player never needs preview, it always exists
end

-- TUIFrame wrappers for Layout integration
local unitFrameWrappers = {}  -- { [unit] = TUIFrame instance }
local containerWrappers = {}  -- { [containerType] = TUIFrame instance }
...
local function MaskBlizzardFrame(unit)
    ...
    blizzardFramesMasked[unit] = {
        ...
        originalMouseClickEnabled = (frame.IsMouseClickEnabled and frame:IsMouseClickEnabled()) or true,
        originalMouseMotionEnabled = (frame.IsMouseMotionEnabled and frame:IsMouseMotionEnabled()) or true,
    }
    
    frame:SetAlpha(0)
    
    -- Disable mouse interaction to prevent tooltips and clicks on hidden frame
    ...
    -- Hook to maintain alpha, mouse state, and keep Edit Mode selection hidden
...
local function UnmaskBlizzardFrame(unit)
    ...
    frame:EnableMouse(data.originalMouseEnabled ~= false)
    if frame.SetMouseClickEnabled then
        frame:SetMouseClickEnabled(data.originalMouseClickEnabled ~= false)
    end
    if frame.SetMouseMotionEnabled then
        frame:SetMouseMotionEnabled(data.originalMouseMotionEnabled ~= false)
    end
...
local function CreateCustomUnitFrame(unit)
    ...
    local function CreateAuraIcon(parent, index)
        -- Use Frame instead of Button - we don't need click handling, just tooltips
        local icon = CreateFrame("Frame", nil, parent, "BackdropTemplate")
        icon:SetFrameLevel(parent:GetFrameLevel() + 1)
        icon:EnableMouse(true)  -- For tooltips only
        ...
        icon.cooldown:SetHideCountdownNumbers(true)  -- We show our own text
        ...
        -- Duration text (on overlay so it's above sweep)
            ...
            if frame.isEmpoweredCast and frame.empowerSpellID then
                if C_Spell and C_Spell.GetSpellInfo then
                    ...
                    if success and spellInfo and spellInfo.name then
                        local chName, _, _, chStart, chEnd = UnitChannelInfo(frame.unit)
    ...
    local function LayoutAuraContainer(container, auraSettings, frameRef)
        ...
        for i, icon in ipairs(container.icons) do
            if i <= maxAuras then
                ...
                icon.durationText:SetFont(GetFontPath(DEFAULT_FONT), auraSettings.durationFontSize or 8, GetFontOutline())
                icon.durationText:ClearAllPoints()
                local durPos = auraSettings.durationPosition or "BOTTOM"
                if durPos == "TOP" then
                    icon.durationText:SetPoint("TOP", 0, -1)
                elseif durPos == "CENTER" then
                    icon.durationText:SetPoint("CENTER", 0, 0)
                else
                    icon.durationText:SetPoint("BOTTOM", 0, -1)
                end
            ...
            if not castName then
                ...
                if frame.isEmpoweredCast and frame.empowerSpellID then
                    if C_Spell and C_Spell.GetSpellInfo then
                        local success, spellInfo = pcall(function() return C_Spell.GetSpellInfo(frame.empowerSpellID) end)
                        if success and spellInfo and spellInfo.name then
                            local chName, _, _, chStart, chEnd = UnitChannelInfo(unit)
                            if chStart and chEnd then
                                castName = spellInfo.name
                                startTime = chStart
                                endTime = chEnd
        ...
        for i, auraData in ipairs(auras) do
            ...
            if icon and auraData then
                ...
                if not textureSet then
                    ...
                    -- Duration text (independent from sweep)
                    -- Default to true if setting doesn't exist (new setting)
                    local showDurationText = auraSettings.showDurationText
                    if showDurationText == nil then showDurationText = true end
                    
                    if showDurationText and auraData.auraInstanceID then
                        -- Update font size and position
                        local fontPath = auraSettings.durationFont and GetFontPath(auraSettings.durationFont) or GetFontPath(DEFAULT_FONT)
                        local fontOutline = auraSettings.durationOutline or "OUTLINE"
                        icon.durationText:SetFont(fontPath, auraSettings.durationFontSize or 10, fontOutline)
                        icon.durationText:ClearAllPoints()
                        local durationAnchor = auraSettings.durationAnchor or "CENTER"
                        local durationOffsetX = auraSettings.durationOffsetX or 0
                        local durationOffsetY = auraSettings.durationOffsetY or 0
                        icon.durationText:SetPoint(durationAnchor, durationOffsetX, durationOffsetY)
                        
                        -- Apply duration text color
                        local durationColor = auraSettings.durationColor or { 1, 1, 0.6, 1 }
                        icon.durationText:SetTextColor(durationColor[1] or 1, durationColor[2] or 1, durationColor[3] or 0.6, durationColor[4] or 1)
                        
                        -- Use TweaksUI.API compatibility layer for duration
                        -- NOTE: In Midnight, remaining may be a SECRET VALUE that cannot be compared
                        local remaining = TweaksUI.API.GetAuraDurationRemaining(unitToken, auraData.auraInstanceID)
                        
                        -- Use TruncateWhenZero which handles secrets - returns empty string for 0/nil
                        if C_StringUtil and C_StringUtil.TruncateWhenZero then
                            local text = C_StringUtil.TruncateWhenZero(remaining)
                            icon.durationText:SetText(text)
                            icon.durationText:Show()
                        elseif remaining then
                            -- Pre-Midnight fallback where remaining is a real number we can compare
                            local success, result = pcall(function()
                                if remaining > 0 then
                            ...
                            if success and result then
                                icon.durationText:SetText(result)
                                icon.durationText:Show()
                            else
                                icon.durationText:Hide()
                            end
...
local function HideBlizzardPartyFrames()
    ...
    -- Hide the main PartyFrame container visually using ONLY SetAlpha(0)
    -- Do NOT use SetScale, ClearAllPoints, or Hide - these can stop aura updates
    if PartyFrame then
        PartyFrame:SetAlpha(0)
        PartyFrame:EnableMouse(false)
        -- NOTE: Keep frame in place and at full scale so aura updates continue
        
        -- Make member frames invisible but keep them functional
        for i = 1, 4 do
            local memberFrame = PartyFrame["MemberFrame" .. i]
            if memberFrame then
                memberFrame:SetAlpha(0)
                memberFrame:EnableMouse(false)
                -- Do NOT unregister events, Hide(), or change scale
            end
        end
    end
    
    -- Also hide CompactPartyFrame (raid-style party frames) - ONLY with alpha
    if CompactPartyFrame then
        CompactPartyFrame:SetAlpha(0)
        CompactPartyFrame:EnableMouse(false)
        -- Do NOT Hide() or SetScale - need aura updates to continue
    end
    ...
    -- REMOVED: Do NOT modify EditModeManagerFrame's registered frames
    -- This was causing taint when Edit Mode tried to enter
    -- Edit Mode needs these frames in their natural state
end
            ...
            if not castName then
                ...
                if frame.isEmpoweredCast and frame.empowerSpellID then
                    if C_Spell and C_Spell.GetSpellInfo then
                        local success, spellInfo = pcall(function() return C_Spell.GetSpellInfo(frame.empowerSpellID) end)
                        if success and spellInfo and spellInfo.name then
                            local chName, _, _, chStart, chEnd = UnitChannelInfo(unitID)
                            if chStart and chEnd then
                                castName = spellInfo.name
                                startTime = chStart
                                endTime = chEnd
        ...
        for i, auraData in ipairs(auras) do
            ...
            if icon and auraData then
                ...
                if not textureSet then
                    ...
                    if showDurationText and auraData.auraInstanceID and icon.durationText then
                        local fontPath = auraSettings.durationFont and GetFontPath(auraSettings.durationFont) or GetFontPath(DEFAULT_FONT)
                        local fontOutline = auraSettings.durationOutline or "OUTLINE"
                        icon.durationText:SetFont(fontPath, auraSettings.durationFontSize or 10, fontOutline)
                        icon.durationText:ClearAllPoints()
...
function UnitFrames:UpdatePartyFrames()
    ...
    if not InCombatLockdown() then
        ...
        if Sorting and sortSettings and sortSettings.enabled then
            sortedUnits = Sorting:GetSortedUnitList(PARTY_UNITS, sortSettings)
        end
            ...
            if icon and auraData then
                ...
                if not textureSet then
                    ...
                    if showDurationText and auraData.auraInstanceID and icon.durationText then
                        ...
                        elseif remaining then
                            local success, result = pcall(function()
                                if remaining > 0 then return string.format("%.0f", remaining) end
                                return nil
                            end)
                            if success and result then
                                icon.durationText:SetText(result)
...
-- Hide Blizzard's boss frames when our custom ones are enabled
local function HideBlizzardBossFrames()
    ...
    if bs and bs.enabled then
        for i = 1, 5 do
            ...
            if frame then
                -- Only modify protected frames outside of combat
                if not InCombatLockdown() then
                    frame:SetAlpha(0)
                ...
                if not frame._tweaksBossHooked then
                    frame:HookScript("OnShow", function(self)
                        if settings and settings.boss and settings.boss.enabled and not InCombatLockdown() then
                            self:SetAlpha(0)
                            self:SetScale(0.001)
                            self:EnableMouse(false)
                        end
        ...
        -- Restore Blizzard frames if our module is disabled
        for i = 1, 5 do
            local frame = _G["Boss" .. i .. "TargetFrame"]
            if frame and not InCombatLockdown() then
                frame:SetAlpha(1)
...
function UnitFrames:HidePanelPreview(panelName)
    ...
    if individualUnits[panelName] then
        ...
        if panelName == "target" then previewTarget = false
...
function UnitFrames:CreateUnitPanel(unit)
    ...
    local function BuildCategoryUI(contentFrame, categoryId)
        ...
        showDurationText = false,
                ...
                CreateSlider("Icon Size:", buffSettings, "size", 10, 32, 1, false, function()
                    if isParty then
                        module:UpdatePartyFrames()
                    elseif isRaid then
                        module:UpdateRaidFrames()
        ...
        showDurationText = false,
                ...
                CreateSlider("Icon Size:", debuffIconSettings, "size", 10, 32, 1, false, function()
                    if isParty then
                        module:UpdatePartyFrames()
                    elseif isRaid then
                        module:UpdateRaidFrames()
            ...
            if visualSettings.buffs then
                ...
                CreateSlider("Icon Size:", visualSettings.buffs, "size", 12, 32, 1)
            ...
            if visualSettings.debuffs then
                ...
                CreateSlider("Icon Size:", visualSettings.debuffs, "size", 12, 32, 1)
        ...
        showDurationText = false,
            ...
            CreateSlider("Icon Size:", debuffSettings, "size", 10, 32, 1, false, function()
                if isParty then module:UpdatePartyFrames()
                elseif isRaid then module:UpdateRaidFrames() end
            end)
...
function UnitFrames:CreateGeneralSettingsPanel()
    ...
    durInfo:SetText("|cff888888Aura icons change color as they expire (green→red)|r")
    yPos = yPos - 24
