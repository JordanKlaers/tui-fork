        ...
        for _, frame in pairs(frames) do
            if frame.unit and UnitIsUnit(frame.unit, unit) then
                ...
                if TweaksUI.db and TweaksUI.db.unitFrames and TweaksUI.db.unitFrames.party then
                    settings = TweaksUI.db.unitFrames.party.healPrediction
                end
    ...
    elseif cmd == "test" then
        ...
        if UnitFrames then
            ...
            if playerFrame then
                ...
                -- Check settings
                local db = TweaksUI.db and TweaksUI.db.unitFrames
                if db then
                    local settings = db.player and db.player.healPrediction
                    if settings then
