...
SlashCmdList["TUITEST"] = function(msg)
    ...
    if cmd == "on" or cmd == "enable" then
        ...
        print("|cff00ff00TweaksUI:|r Test mode enabled with " .. count .. " frames")
        
        -- Check if party container exists and provide helpful message
        local partyContainer = _G["TweaksUI_PartyContainer"]
        if not partyContainer or not partyContainer:IsShown() then
            print("|cffFFFF00Note:|r Party frames may not be visible.")
            print("  Make sure Party Frames are enabled in TweaksUI settings:")
            print("  /tui → UnitFrames → Party → Enable")
        end
        
    elseif cmd == "off" or cmd == "disable" then
        TestMode:Disable()
        print("|cff00ff00TweaksUI:|r Test mode disabled")
        
    elseif cmd == "toggle" then
        TestMode:Toggle()
        print("|cff00ff00TweaksUI:|r Test mode " .. (TestMode.enabled and "enabled" or "disabled"))
    ...
    elseif cmd == "feature" then
        ...
        print("  /tuitest [toggle] - Toggle test mode")
        print("  /tuitest on [count] - Enable with frame count")
        print("  /tuitest off - Disable test mode")
