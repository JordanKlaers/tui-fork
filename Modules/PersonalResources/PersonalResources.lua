...
local settings, personalResourcesHub, settingsPanels, currentOpenPanel, previewWindow = nil, nil, {}, nil, nil
...
local function CreateAuraIcon(parent, index)
    ...
    -- Duration text (centered by default)
    ...
    -- OnUpdate handler for duration text continuous refresh
    icon:SetScript("OnUpdate", function(self, elapsed)
        if not self.showDurationText or not self.auraInstanceID then
        ...
        local remaining = TweaksUI.API.GetAuraDurationRemaining("player", self.auraInstanceID)
        
        -- Use TruncateWhenZero which handles secrets - returns empty string for 0/nil
        if HasStringTruncate() then
        ...
        elseif remaining then
            ...
            if success and result then
                self.durationText:SetText(result)
                self.durationText:Show()
            else
                self.durationText:Hide()
            end
...
local function LayoutAurasStandard(container, cfg, numAuras)
    ...
    for i, icon in ipairs(container.icons) do
        if i <= maxAuras then
            ...
            icon.durationText:SetFont(STANDARD_TEXT_FONT, cfg.durationFontSize or 10, "OUTLINE")
            icon.durationText:ClearAllPoints()
            if cfg.durationPosition == "TOP" then
                icon.durationText:SetPoint("TOP", 0, -2)
            elseif cfg.durationPosition == "BOTTOM" then
                icon.durationText:SetPoint("BOTTOM", 0, 2)
            else
                -- Default: CENTER
                icon.durationText:SetPoint("CENTER", 0, 0)
            end
...
local function UpdateAuraContainer(container, cfg, isBuff)
    ...
    -- Try GetUnitAuraInstanceIDs with sorting (Midnight Beta 4+)
    if HasAuraSorting() and C_UnitAuras.GetUnitAuraInstanceIDs then
        -- Map config sortBy to Enum.UnitAuraSortRule
        local sortRule = Enum.UnitAuraSortRule.Default
        if cfg.sortBy == "duration" then
            sortRule = Enum.UnitAuraSortRule.Expiration
        elseif cfg.sortBy == "name" then
            sortRule = Enum.UnitAuraSortRule.Name
        end
        
        -- Map config sortDirection to Enum.UnitAuraSortDirection
        local sortDirection = Enum.UnitAuraSortDirection.Normal
        if cfg.sortDirection == "reverse" then
            sortDirection = Enum.UnitAuraSortDirection.Reverse
        end
        
        local success, auraIDs = pcall(function()
            return C_UnitAuras.GetUnitAuraInstanceIDs("player", filter, maxAuras, sortRule, sortDirection)
        end)
        
        if success and auraIDs then
            for _, auraID in ipairs(auraIDs) do
                local auraSuccess, auraData = pcall(function()
                    return C_UnitAuras.GetAuraDataByAuraInstanceID("player", auraID)
                end)
    ...
    if #auras == 0 and AuraUtil and AuraUtil.ForEachAura then
        pcall(function()
            AuraUtil.ForEachAura("player", filter, maxAuras, function(aura)
    ...
    for i, auraData in ipairs(auras) do
        ...
        if icon and auraData then
            ...
            -- Duration text - set directly like nameplates do
            -- Uses GetAuraDurationRemaining which returns secret that string.format can handle
            icon.durationText:SetFont(STANDARD_TEXT_FONT, cfg.durationFontSize or 10, "OUTLINE")
            icon.durationText:ClearAllPoints()
            if cfg.durationPosition == "TOP" then
                icon.durationText:SetPoint("TOP", 0, -2)
            ...
            if cfg.showDurationText and auraData.auraInstanceID then
                -- Use TweaksUI.API compatibility layer for duration
                -- NOTE: In Midnight, remaining may be a SECRET VALUE that cannot be compared
                local remaining = TweaksUI.API.GetAuraDurationRemaining("player", auraData.auraInstanceID)
            ...
            -- Store for OnUpdate continuous refresh (if APIs available)
            icon.showDurationText = cfg.showDurationText
            icon.auraInstanceID = auraData.auraInstanceID
...
local function RefreshAllBars()
    ...
    UpdateBuffs()
    UpdateDebuffs()
    if previewWindow and previewWindow:IsShown() then PersonalResources:UpdatePreview() end
end
...
-- Preview Window
function PersonalResources:CreatePreviewWindow()
    if previewWindow then previewWindow:Show() self:UpdatePreview() return end
    local w = CreateFrame("Frame", "TweaksUI_PersonalResources_Preview", UIParent, "BackdropTemplate")
    w:SetSize(400, 300)
    ...
    title:SetText("Personal Resources Preview")
    title:SetTextColor(1, 0.82, 0)
    
    local closeBtn = CreateFrame("Button", nil, w, "UIPanelCloseButton")
    closeBtn:SetPoint("TOPRIGHT", -3, -3)
    closeBtn:SetScript("OnClick", function() w:Hide() if personalResourcesHub and personalResourcesHub.previewBtn then personalResourcesHub.previewBtn:SetText("Show Preview") end end)
...
function PersonalResources:UpdatePreview()
    ...
    elseif rt == SPECIAL_RESOURCES.VOID_METAMORPHOSIS then rn = "Void Metamorphosis"
    elseif rt == SPECIAL_RESOURCES.MAELSTROM_WEAPON then rn = "Maelstrom Weapon"
...
function PersonalResources:TogglePreviewWindow()
    if previewWindow and previewWindow:IsShown() then
        previewWindow:Hide()
        if personalResourcesHub and personalResourcesHub.previewBtn then personalResourcesHub.previewBtn:SetText("Show Preview") end
    else
        self:CreatePreviewWindow()
        if personalResourcesHub and personalResourcesHub.previewBtn then personalResourcesHub.previewBtn:SetText("Hide Preview") end
    end
end
