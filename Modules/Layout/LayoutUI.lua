...
function LayoutUI:StartDrag(elementId)
    ...
    local SnapLocking = TweaksUI.SnapLocking
    if SnapLocking and SnapLocking:IsInLockedGroup(elementId) then
...
-- Find the nearest anchor points between two frames
-- Returns: draggedAnchor, targetAnchor, distance
-- Uses preference PENALTIES to favor edge-to-edge over corner-to-corner
function LayoutUI:FindNearestAnchorPoints(draggedFrame, targetFrame)
    ...
    local closestDragAnchor = nil
    local closestTargetAnchor = nil
    local closestWeightedDist = 9999
    local closestRawDist = 9999
    
    for dragAnchor, dp in pairs(dragPoints) do
        for targetAnchor, tp in pairs(targetPoints) do
            local rawDist = math.sqrt((dp.x - tp.x)^2 + (dp.y - tp.y)^2)
            local penalty = GetPreferencePenalty(anchorTypes[dragAnchor], anchorTypes[targetAnchor])
            local weightedDist = rawDist + penalty
            
            if weightedDist < closestWeightedDist then
                closestWeightedDist = weightedDist
                closestRawDist = rawDist
                closestDragAnchor = dragAnchor
                closestTargetAnchor = targetAnchor
            end
        end
    end
    
    return closestDragAnchor, closestTargetAnchor, closestRawDist
end
