...
local function GetUninterruptibleColorCurve()
    ...
    if CreateColorCurve then
        ...
        -- Add control points: value, r, g, b, a
...
local function ApplyCastBarColor(statusBar, notInterruptible, baseColor, isPlayerUnit)
    ...
    if HAS_COLOR_FROM_BOOLEAN and notInterruptible ~= nil then
        ...
        if curve then
            ...
            if ok and colorResult then
                -- colorResult is a ColorMixin - check if it's grey (uninterruptible)
                -- If r ~= 1.0, it's the grey color from our curve
    ...
    -- Fallback: can't determine, use base color but could be wrong
    statusBar:SetStatusBarColor(baseColor[1], baseColor[2], baseColor[3], baseColor[4] or 1)
    return false
end
...
local function HideSpellBarWithHooks(spellBar, settingsKey)
    ...
    if not spellBar._tweaksHooked then
        hooksecurefunc(spellBar, "Show", function(self)
            if settings[settingsKey] and settings[settingsKey].enabled and settings[settingsKey].hideBlizzard then
                self:SetAlpha(0)
                self:SetScale(0.001)
            end
        end)
        spellBar:HookScript("OnShow", function(self)
            if settings[settingsKey] and settings[settingsKey].enabled and settings[settingsKey].hideBlizzard then
                self:SetAlpha(0)
                self:SetScale(0.001)
            end
...
local function HideBlizzardCastBars()
    -- Player cast bar
    if settings.player and settings.player.enabled and settings.player.hideBlizzard then
    ...
    -- Target cast bar
    if settings.target and settings.target.enabled and settings.target.hideBlizzard then
    ...
    -- Focus cast bar
    if settings.focus and settings.focus.enabled and settings.focus.hideBlizzard then
