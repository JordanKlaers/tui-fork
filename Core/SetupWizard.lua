...
local selectedProfile = "Basic"  -- Default to built-in Basic profile
...
local function BuildStep3(container)
    ...
    UIDropDownMenu_SetText(dropdownFrame, "Basic (Built-in)")
    
    UIDropDownMenu_Initialize(dropdownFrame, function(self, level)
        -- Built-in Basic profile (always first)
        local basicInfo = UIDropDownMenu_CreateInfo()
        basicInfo.text = "Basic (Built-in)"
        basicInfo.value = "Basic"
        basicInfo.checked = (selectedProfile == "Basic")
    ...
    desc:SetText("The |cff00ff00Basic|r profile enables all features with sensible defaults.\nSaved profiles from other characters appear in the dropdown.")
